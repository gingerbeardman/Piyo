...
b=null},xmlHttpRequest:function(e,g){var c=function(d){if(EMV){console.log("emu: req ("+j+" -"+e.url+" returned: "+d)
}if(d.onLoad&&g.callback){if(d.r_id){d.response.response=window[d.r_id];
delete window[d.r_id]}g.callback(d.response)}if(d.onReadyStateChange&&g.onreadychange){g.onreadychange(d.response)
}if(d.onProgress&&g.onprogress){g.onprogress(d.response)
}if(d.onError&&g.onerr){g.onerr(d.response)}if(d.onDone&&g.done){g.done(d.response)
}};var j=b.getResponseId(c);var i;if(typeof e.data==="object"){var f="__y__"+Math.floor(Math.random()*6121983+1);
window[f]=e;i=JSON.stringify({q_id:f})}else{i=JSON.stringify(e)
}var h=function(){tmCE.xmlHttpRequest(b.key,i,j)};window.setTimeout(h,1)
